...
const PaidConfirmation = ({ cart, total, onRemoveAll }) => {
  return (
    <section className='absolute inset-0 py-40 px-6 bg-gray-900 bg-opacity-50'>
      <article className='grid p-7 py-10 gap-6 t:w-3/5 t:mx-auto d:w-2/5 d:mx-auto bg-white rounded-lg'>
        <div className='grid h-16 w-16 rounded-full bg-moderate-orange'>
        ...
        <article className='grid t:grid-cols-2 text-sm'>
          {cart.length > 0 && (
            <article className='grid sectionide-y sectionide-gray-300 gap-4 p-5 bg-light-gray rounded-t-lg t:rounded-t-none t:rounded-l-lg'>
              <article className='grid grid-cols-4 items-center justify-items-center'>
                <div
                  className={`bg-no-repeat bg-center bg-cover bg-${cart[0].id} h-12 w-12 rounded-lg`}
                ></div>
                <article className='grid t:ml-4 col-span-2 text-left'>
          ...
          <article className='grid p-6 gap-3 bg-black rounded-b-lg t:rounded-bl-none t:rounded-r-lg'>
