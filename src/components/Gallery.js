const Gallery = ({ product }) => {
  return (
    <main className='grid t:grid-cols-9 t:grid-rows-2 gap-8'>
      <section
      ...
      ></section>
      <section
        className={`t:col-span-5 t:row-end-3 t:row-span-2 rounded-lg bg-no-repeat bg-center bg-cover bg-m-${product.slug}-gallery-3 t:bg-t-${product.slug}-gallery-3 d:bg-d-${product.slug}-gallery-3 h-96 t:h-full w-full`}
      ></section>
    </main>
  );
};
